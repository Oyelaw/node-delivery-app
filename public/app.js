  ...
  if(phone){
    ...
    app.client.request(undefined, 'api/users', 'GET', queryStringObject, undefined, function(statusCode, responsePayload){
      if(statusCode == 200){
        ...
        console.log('Length ord', allOrders.length);
        if(allOrders.length > 0){
          // Show each order created as a new row in the table
          allOrders.forEach(function(orderId){
            ...
            app.client.request(undefined, 'api/orders', 'GET', newQueryStringObject, undefined, function(statusCode, responsePayload){
              if(statusCode == 200){
                ...
                td0.innerHTML = responsePayload.deliveryPhone;
                td1.innerHTML = responsePayload.deliveryAddress;
                td2.innerHTML = responsePayload.pickupPhone;
                td3.innerHTML = responsePayload.pickupAddress;
                var state = typeof(responsePayload.state) == 'string' ? responsePayload.state : 'unknown';
                td4.innerHTML = state;
                td5.innerHTML = '<a href="/orders/edit?id='+responsePayload.id+'">View / Edit / Delete</a>';
              } else {
  ...
  if(id){
    ...
    app.client.request(undefined, 'api/orders', 'GET', queryStringObject, undefined, function(statusCode, responsePayload){
      if(statusCode == 200){
        ...
        document.querySelector("#ordersEdit1 .deliveryPhone").value = responsePayload.deliveryPhone;
        document.querySelector("#ordersEdit1 .deliveryAddress").value = responsePayload.deliveryAddress;
        document.querySelector("#ordersEdit1 .pickupPhone").value = responsePayload.pickupPhone;
        document.querySelector("#ordersEdit1 .pickupAddress").value = responsePayload.pickupAddress;
